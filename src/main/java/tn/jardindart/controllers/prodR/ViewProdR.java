...
public class ViewProdR implements Initializable {
    ...
    private void rechercherAssociations(KeyEvent event) {
        ...
        List<ProdR> associations = prodRService.recupererParUtilisateur(userId);

        if (!searchTerm.isEmpty()) {
            associations = associations.stream()
                    .filter(a -> a.getNomP().toLowerCase().contains(searchTerm.toLowerCase())
                            || a.getNomTypeProd().toLowerCase().contains(searchTerm.toLowerCase()))
        ...
        for (ProdR association : associations) {
