...
public class CardView implements Initializable {
    ...
    private void displayDonsBienMateriel() {
        ...
        List<DonBienMateriel> donsBienMateriel = service.afficherParUtilisateur(userId);
    ...
    private AnchorPane createCard(DonBienMateriel don) {
        ...
        JFXButton editButton = createIconButton("/tn.jardindart/icons/edit.png", 20);
        editButton.setOnAction(event -> handleEditDon(don));
        actionsBox.getChildren().add(editButton);

        JFXButton deleteButton = createIconButton("/tn.jardindart/icons/delete.png", 20);
    ...
    private void handleEditDon(DonBienMateriel don) {
        ...
        FXMLLoader fxmlLoader = new FXMLLoader(HelloApplication.class.getResource("/tn.jardindart/DBM/UpdateDBM.fxml"));
        try {
        ...
        alert.showAndWait().ifPresent(buttonType -> {
            if (buttonType == ButtonType.OK) {
                try {
                    ...
                    refreshView();
                   // HelloApplication.updateCurrentView("/tn.jardindart/DBM/card_view.fxml");
                } catch (Exception e) {
